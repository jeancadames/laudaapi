...
class ServiceLaunchController extends Controller
{
    ...
    public function open(Request $request, Service $service): RedirectResponse
    {
        ...
        if ($launchMode === 'internal') {
            return $this->openInternal($service);
        }

        if ($launchMode === 'external') {
            return $this->openExternal($request, $user, $company, $service);
        }

        if ($launchMode === 'embedded') {
            return $this->openEmbedded($service);
        }

        if ($launchMode === 'api') {
            return $this->openApi($service);
        }

        throw new HttpException(500, 'launch_mode no soportado.');
    }
    ...
    private function openExternal(Request $request, $user, Company $company, Service $service): RedirectResponse
    {
