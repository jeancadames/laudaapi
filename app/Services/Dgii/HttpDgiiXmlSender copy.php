...
logger()->info('DGII token extracted', [
    'company_id' => $companyId,
    'environment' => $environment,
    'token_len' => strlen(trim($token)),
    'token_prefix' => mb_substr(trim($token), 0, 30),
]);
        if (trim($token) === '') {
            $tx->status = 'failed';
        ...
        try {
            ...
            $res = Http::timeout($timeout)
                ...
                ], $this->withoutContentTypeHeader($extraHeaders)))
                ->attach($multipartField, $xml, $finalFilename, [
                    'Content-Type' => 'text/xml',
                ])
                ->send($method, $url);

            logger()->info('DGII XML send payload', [
                'company_id' => $companyId,
                'environment' => $environment,
