          ...
          >
            <div class="flex flex-col items-center">
              ...
              <StepperSeparator
                ...
                class="my-1 h-8 w-px bg-muted group-data-[state=completed]:bg-primary"
              />
            </div>
        ...
        <CardContent>
          <div v-if="!isReview" class="space-y-5">
            ...
            >
              ...
              <RadioGroup
                :model-value="answers[question.id] ? String(answers[question.id]) : undefined"
                class="mt-4 grid gap-2"
                :disabled="readOnly"
                @update:model-value="(value) => answers[question.id] = Number(value)"
              >
                <Label
                  v-for="choice in question.choices"
                  :key="choice.score"
                  :for="`${question.id}-${choice.score}`"
                  class="flex cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm font-normal leading-5 transition hover:bg-muted/50"
                  :class="answers[question.id] === choice.score && 'border-primary bg-primary/5'"
                >
                  <RadioGroupItem
                    :id="`${question.id}-${choice.score}`"
              ...
              <div class="mt-4">
                ...
                <Textarea
                  ...
                  placeholder="Contexto, evidencia o aclaración para esta respuesta."
                />
              </div>
            </section>
          </div>
